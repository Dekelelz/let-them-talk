<p align="center">
  <img src="agent-bridge/logo.png" alt="Let Them Talk" width="140">
</p>
...
```bash
npm test                           # delegates to agent-bridge
npm --prefix agent-bridge run verify            # full suite
npm --prefix agent-bridge run verify:contracts  # runtime + schema + branches + markdown
npm --prefix agent-bridge run verify:replay     # event replay (healthy + clean + negative)
npm --prefix agent-bridge run verify:invariants # dashboard, capabilities, parity, sessions, evidence, autonomy, hooks
npm --prefix agent-bridge run verify:smoke      # representative subset
```
...
- **0 known vulnerabilities** in the shipped tarball as of v5.5.1.
