...
Current-code anchors: `agent-bridge/state/canonical.js`, `agent-bridge/events/log.js`, `agent-bridge/scripts/check-migration-hardening.js`  
...
| --- | --- | --- |
| Legacy `messages.jsonl` or `history.jsonl` can stand in for a missing canonical branch event log during rebuild | Invalid. A missing canonical branch stream with surviving legacy projections is a fail-closed condition. | `agent-bridge/state/canonical.js::rebuildMessageProjections()` plus `agent-bridge/scripts/check-migration-hardening.js` |
...
| Rollback can rewrite append-only canonical history in place | Invalid. Rollback uses compensating events, projection rebuild, or pre-migration backup restore. | `docs/architecture/runtime-contract.md` and `agent-bridge/scripts/check-migration-hardening.js` |
| Message-only branch switching is enough to claim migration cutover is hardened | Invalid. Full branch-local state isolation stays required. | `docs/architecture/branch-semantics.md` and `agent-bridge/scripts/check-branch-isolation.js` |
| Unsupported storage formats may keep writing canonically if a projection looks readable | Invalid. Unknown or newer formats fail closed. | `docs/architecture/runtime-contract.md` and `agent-bridge/scripts/check-migration-hardening.js` |
...
```bash
node agent-bridge/scripts/check-migration-hardening.js
```

Expected-failure legacy-path simulation:

```bash
node agent-bridge/scripts/check-migration-hardening.js --scenario legacy-projection-without-canonical-log
```
