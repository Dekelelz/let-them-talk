...
Current code anchors: `agent-bridge/state/canonical.js`, `agent-bridge/state/dashboard-queries.js`, `agent-bridge/dashboard.js`, `agent-bridge/dashboard.html`  
...
Task 9B should assemble markdown through `agent-bridge/state/canonical.js` and add thin read-only wrappers there for domains that still live in legacy compatibility files today.
...
The best low-churn export assembly point is `agent-bridge/state/canonical.js`.
...
```bash
node agent-bridge/scripts/check-markdown-workspace.js
```
